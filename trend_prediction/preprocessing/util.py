...
def sliding_window(X, window_size):
	if len(X.shape) == 1:
		shape = (window_size, )
		out_shape = (-1, window_size)
	else:
		shape = (window_size, X.shape[-1])
		out_shape = (-1, X.shape[-1]*window_size)
		# The last feature does not have a target
	return _sliding_window_view(X, shape).reshape(out_shape)[:-1]


def targets(time_series, window=1):
	if len(time_series.shape) < 2:
		shift = (-window)
	else:
		n_columns = time_series.shape[-1]
		shift = tuple([-window for _ in range(n_columns)])
	_targets = np.roll(time_series, shift=shift)[:-window]
	return _targets.reshape(_targets.shape[0], -1)
...
def _sliding_window_view(x, shape, step=None, subok=False, writeable=False):
	"""
